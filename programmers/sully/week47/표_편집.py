...
def solution(n: int, k: int, cmd: List[str]) -> str:
    ...
    for c in cmd:
        ...
        if c == 'C':
            ...
            # 삭제된 행 스택 추가
            remove_stack.append([prev, current, nxt])
        ...
        if c == 'Z':
            # 가장 최근에 삭제된 행 복구
            restored_prev, restored_current, restored_nxt = remove_stack.pop()
            answer[restored_current] = 'O'

            # 복구된 행의 이전과 다음 행의 인덱스 복구
            # 복구된 행이 첫 번째 행인 경우니 다음 행의 이전 행을 현재 행으로 설정
            if restored_prev is None:
                table[restored_nxt][0] = restored_current
            # 복구된 행이 마지막 행인 경우니 이전 행의 다음 행을 현재 행으로 설정
            elif restored_nxt is None:
                table[restored_prev][1] = restored_current
